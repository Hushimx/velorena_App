...
export default function CategoryScreen() {
  ...
  const renderProductItem = useCallback(({ item }: { item: Product }) => {
    const image = item.image || item.main_image || 
                 ...
                 'https://images.unsplash.com/photo-1586953208448-b95a79798f07?w=400&h=300&fit=crop&crop=center&q=60';
    ...
    return (
      <View style={styles.cardShadowWrap}>
        <TouchableOpacity 
          ...
          activeOpacity={0.85}
          accessible={true}
          accessibilityLabel={`منتج: ${item.name_ar || item.name}`}
        ...
        >
          <Image 
            ...
            accessible={true}
            accessibilityLabel="صورة المنتج"
          />
          <TouchableOpacity 
            style={styles.wishBtn} 
            activeOpacity={0.7}
            accessible={true}
            accessibilityLabel="إضافة إلى المفضلة"
            accessibilityRole="button"
          >
            <MaterialIcons name="favorite-border" size={18} color={BROWN_DARK} />
          </TouchableOpacity>
          <View style={styles.cardInfo}>
            ...
            <View style={styles.priceRow}>
              <Text style={styles.priceText}>
                {item.base_price ? `${item.base_price} ريال` : ''}
              </Text>
            </View>
          </View>
        </TouchableOpacity>
